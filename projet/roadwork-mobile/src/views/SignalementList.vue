    ...
    <ion-content>
      ...
      <ion-card v-if="isAuthenticated">
        <ion-card-content>
          <ion-grid>
            <ion-row>
              <ion-col size="6" v-for="stat in userStats" :key="stat.label">
                <div class="stat-item">
                  <h3>{{ stat.value }}</h3>
                  <p>{{ stat.label }}</p>
                </div>
      ...
      <ion-card v-if="showFilters">
        ...
        <ion-card-content>
          <ion-segment v-model="activeFilter" @ionChange="applyFilter">
            ...
            <ion-segment-button value="status">
              <ion-label>Par statut</ion-label>
            </ion-segment-button>
          </ion-segment>

          <div v-if="activeFilter === 'status'" class="ion-margin-top">
            <ion-chip 
      ...
      <div v-else-if="filteredSignalements.length === 0" class="empty-state">
        ...
        <p v-if="activeFilter === 'my' && isAuthenticated">
...
const selectedStatus = ref(['Nouveau', 'En cours']);
const page = ref(1);
const limit = ref(20);
...
const filteredSignalements = computed(() => {
  ...
  // Filtrer par type
  if (activeFilter.value === 'my' && authStore.user) {
