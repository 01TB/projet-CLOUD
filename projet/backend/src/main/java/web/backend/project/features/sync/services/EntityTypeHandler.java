    ...
    public E updateOrCreate(Map<String, Object> firebaseData, EntityManager entityManager) {
        ...
        if (id != null) {
            ...
            if (existing != null) {
                ...
                // Entity doesn't exist in database, create new one with Firebase ID
                entity = createEntity();
                entity.setId(id);
                isNewEntity = true;
        ...
        try {
            if (isNewEntity) {
                // For new entities: persist with explicit Firebase ID if set,
                // or let DB auto-generate via GENERATED BY DEFAULT AS IDENTITY
